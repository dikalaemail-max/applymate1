...
export default function ScholarshipDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from("scholarship_files").select("*").eq("scholarship_id", id),
    ]).then(([{ data: s }, { data: f }]) => {
      if (s) {
        setScholarship(s);
        setForm({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { error: uploadError } = await supabase.storage.from("scholarship-files").upload(path, file);
    if (uploadError) {
      toast({ title: "Upload failed", description: uploadError.message, variant: "destructive" });
      setUploading(false);
      return;
    }
    await supabase.from("scholarship_files").insert({
      scholarship_id: id, user_id: user.id, file_name: file.name,
      file_path: path, file_size: file.size, mime_type: file.type,
    });
    const { data: f } = await supabase.from("scholarship_files").select("*").eq("scholarship_id", id);
    setFiles(f || []);
    setUploading(false);
  ...
  const handleShare = async () => {
    ...
    if (!scholarship.is_shared) {
      await supabase.from("scholarships").update({ is_shared: true }).eq("id", scholarship.id);
      const { data } = await supabase.from("scholarships").select("*").eq("id", scholarship.id).single();
      if (data) setScholarship(data);
    }
